...
## 功能
操作下拉框选择：乘法 / 加减 / 转置 / 标量乘 / 方阵求逆 / 左逆 / 右逆 / 伪逆 / LU 分解 / 增广求解 / REF（行阶梯形）/ 行列式 / 秩 / 特征值特征向量。

- 行列数：直接输入数字（1–16，默认 3×3），改完即时重绘网格。
- 输入：格子内填数字或分数（如 `1/3`），方向键 / 回车在格间移动（回车按行优先跳下一格）。
- 命名矩阵库：默认 A、B、C、D，可新增 / 改名 / 删除；表达式框里直接用名字调用（与网页版同款）。
- 从图片导入：点「从图片导入」选一张矩阵图片，用 Gemini 视觉模型识别成数字填进网格（识别后请核对）。
  需先在「设置」里填入免费 API Key（aistudio.google.com 获取）；Key 只存本机
  `~/.la_helper_settings.json`（权限 0600），不会上传到任何服务器。
